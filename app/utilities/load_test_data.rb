...
#
    ...
    nodes = hash['omim_gene']
    nodes.keys.each do |key, value|
      omim_gene = GeneUtils.create_omim_gene(key, 'Homo sapiens')
      omim_gene.omim_gene_id = key.to_s
      #omim_gene.save

      # create or update neighbor nodes and create connections
      neighbors = nodes[key]
      neighbors_array = neighbors['omim_disease']

      omim_disease = nil
      if neighbors_array
        neighbors_array.each do |omim_disease_id|
          omim_disease = DiseaseUtils::create_omim_disease(omim_disease_id)
          if omim_disease.save
            #c = NeighborRel.new(from_node: node, to_node: neighborB)
            #c.save
            omim_gene.diseases << omim_disease
          end
        end
      end

      neighbors_array = neighbors['entrez_gene_hs']
      if neighbors_array
        neighbors_array.each do |identifier|
          entrez_gene = GeneUtils::create_entrez_gene(identifier, 'Homo sapiens')
          entrez_gene.diseases << omim_disease unless omim_disease.nil?
          if entrez_gene.save
            omim_gene.genes << entrez_gene
          end
        end
      end
      omim_gene.save
    ...
    puts 'OMIM Genes loaded into database'
  end
end
